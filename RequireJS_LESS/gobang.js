define(["svg", "settings", "js_extension"], function(SVG, settings){
	var len = settings.LEN,
		cell_base_size = settings.CELL_BASE_SIZE,
	...
	return {
		start: function(gb_id){
			// work as initial function
			var gb = SVG(gb_id);
			...
			// add click listener
			document.getElementById(gb_id).addEventListener("click", clickHandler);
			// start game auto
			restart();
		}
	};
